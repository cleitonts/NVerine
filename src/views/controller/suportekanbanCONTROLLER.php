...
class suportekanbanCONTROLLER implements ControladoraCONTROLLER
{
    ...
    public function persist($obj)
    {
        ...
        if(!empty($_REQUEST["novo"])){
            ...
            $chamado->cod_status = $chamado::STATUS_NAO_CONFIRMADO;
        ...
        if(!empty($_REQUEST["onlyStatus"])){
            foreach ($_REQUEST["changes"] as $r){
                ...
                if($chamado->after != $r["after"])
                    $string .= "<p>Alterou a ordem.</p>";

                if($chamado->cod_status != $r["status"])
                    $string .= "<p>Alterou o status para: <strong>".SuporteChamadoETT::getStatusKanban($r["status"]).".</strong></p>";

                $historico = new SuporteHistoricoETT();
                $historico->chamado = $chamado->handle;
                $historico->hora = date("H:i:s");
                $historico->comentarios = $string;
        ...
        if ($chamado->cod_status == SuporteChamadoETT::STATUS_NAO_CONFIRMADO && !empty($_REQUEST["responsavel"])) {
            $chamado->cod_status = SuporteChamadoETT::STATUS_CONFIRMADO;
            $string .= "<p>Alterou o status para: <strong>".SuporteChamadoETT::getStatusKanban(SuporteChamadoETT::STATUS_CONFIRMADO).".</strong></p>";
        }
